...
class KITTIDataset(Dataset):
	...
	def __init__(self, KITTIBaseDir, sequences = None, startFrames = None, endFrames = None, \
		parameterization = 'default', width = 256, height = 256, outputFrame = 'local'):
		...
		self.KITTIMaxFrames = [3400,1044,7000,1421]
	...
	def __getitem__(self, idx):
		...
		curImgDir = os.path.join(self.imgDir , str(seqIdx))
		...
		# check if images are correct; comment if no need to debug
		############################################################
		img_event = np.zeros_like(img1)
		img_event[:,:,0] = event1[0, :,:]*event1[2,:,:]
		img_event[:,:,2] = event1[1,:,:]*event1[3,:,:]
		img2_debug = os.path.join(curImgDir, 'left_image_debug'+str(frame2).zfill(5) + '.png')
		try:
			smc.imsave(img2_debug, img_event)
		except:
		...
		pair = torch.empty([1, 6, self.height, self.width])	
		...
		poses = np.loadtxt(os.path.join(self.poseDir, str(seqIdx).zfill(1) + '.txt'), \
			dtype = np.float32)
	...
	def preprocessImg(self, img):
		...
		img = np.resize(img, (self.height, self.width, 1))
